...
func InfoCmd(cmd *cobra.Command, con *console.SliverClient, args []string) {
	...
	if len(args) > 0 {
		...
		idArg := args[0]
		session, bacon, err = use.SessionOrBeaconByID(idArg, con)
		if err != nil {
			con.PrintErrorf("%s\n", err)
			return
		}
		...
		session, bacon, err = use.SessionOrBeaconByID(currID, con)
		if err != nil {
			con.PrintErrorf("%s\n", err)
			return
		}
		...
		if session == nil && bacon == nil {
			session, bacon, err = use.SelectSessionOrBeacon(con)
			if err != nil {
				con.PrintErrorf("%s\n", err)
				return
			}
